...
(function (root, factory) {
  ...
  /** ★年5日の時季指定義務★（10日以上 付いた人だけ）
   *   ＝★取らせないと 会社が罰を受ける★ので、社長に「あと何日・いつまで」を出すために使う。
   * @param {object} o { hireDate, today, takenDays }
   * @returns {object} { target, needDays, byDate, usedDays, mustDays }
   *   target=false … 対象外（10日未満・入社半年未満・入社日が無い）＝★何も出さない★
   */
  function must5State(o) {
    var r = yukyuLeft(o);
    if (!r.ok || !r.mustTake5) {
      return { target: false, needDays: 0, byDate: '', usedDays: r.usedDays || 0, mustDays: YUKYU_MUST_TAKE_DAYS };
    }
    return {
      target: true,
      needDays: Math.max(0, YUKYU_MUST_TAKE_DAYS - r.usedDays),
      byDate: r.period.to,
      usedDays: r.usedDays,
      mustDays: YUKYU_MUST_TAKE_DAYS,
    };
  }

  return {
    must5State: must5State,
