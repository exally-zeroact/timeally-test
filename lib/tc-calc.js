...
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(require('./tc-law.js'), require('./tc-clean.js'));
  } else {
    root.TcCalc = factory(root.TcLaw, root.TcClean);
  }
})(typeof self !== 'undefined' ? self : this, function (LAW, CLEAN) {
  'use strict';
  ...
  /* ── ① 生データを日ごとに組み立てる ─────────────────────────
     ...
     ★2026-08-18 追加★: 組み立てる前に ★lib/tc-clean.js に掃除させる★。
       ・連打（同じ分／窓の中に続く同じ種類）は ★後の1本だけ★を使う
       ・同じ分に 出勤と退勤 が並んだ日は ★どちらか決められない★ので どちらも使わず
         ★「決められない」と言う★（★勝手に0にして黙る★のが いちばん怖い）
       ★判定を ここに書き直さない★＝従業員の画面も 同じ tc-clean を読む（1か所）。 */
  function sessions(punches, opts) {
    var cleaned = CLEAN.clean(punches, opts || {});
    var ps = cleaned.used;
    ...
    return { list: out, stray: stray, clean: cleaned };
  }
  ...
  function summarize(arg) {
    ...
    /* ★today は「今日はまだ働いている」を聞かない為だけに使う★（省略してよい・省くと全部 聞く） */
    var ses = sessions(arg.punches, { today: arg.today || null });
    ...
    while (cursor <= per.to) {
      ...
      days.push({
        ...
        /* ★2026-08-18★ 打った物の掃除の結果（lib/tc-clean.js が決めた物をそのまま持つ）
           undecided … ★この日は決められない★（同じ分に出勤と退勤）＝0にして黙らない
           merged    … 連打として1本にまとめた本数（★画面には薄く残す★）
           asks      … 画面が そのまま出す「聞く事」（文も tc-clean が持つ＝1か所） */
        undecided: false, merged: 0, asks: [],
    ...
    /* ★掃除の結果を その日に載せる★（★数える所と 画面が 同じ物を見る★）
       ★決められない日は 0分のまま黙らせない★＝undecided を立てて 画面・紙・CSVで言う。 */
    Object.keys(ses.clean.byDay).forEach(function (dd) {
      var d = byDate[dd];
      if (!d) return;                                     // 期間外（前後の月）
      var c = ses.clean.byDay[dd];
      d.undecided = c.undecided;
      d.merged = c.merged;
      d.asks = c.asks;
      if (c.undecided) d.incomplete = true;
    });
    /* ★時刻そのものが違うかもしれない所★（2026-08-18 夜）＝長すぎ／短すぎ／退勤が先。
       ★線は会社の1日の決まりから作る★（従業員の画面と同じ線・判定は tc-clean の1本）。 */
    CLEAN.timeIssues(arg.punches, { today: arg.today || null, dayStdMin: o.dailyStdMin })
      .forEach(function (x) {
        var dd = byDate[dayOf(toMin(x.at))];
        if (!dd) return;
        dd.asks = dd.asks.concat([{ d: dd.d, type: x.type, at: x.at, hm: x.at.slice(11, 16), text: x.text }]);
      });
    ...
    /* ★決められない日の数★（2026-08-18）… ★0にして黙る★の反対側。
       ★給与へ渡す前に 社長がこれを見る★（数字が小さいのに理由が分からない、を作らない） */
    month.undecidedDays = days.filter(function (d) { return d.undecided; }).length;
    month.askCount = days.reduce(function (a, d) { return a + (d.asks ? d.asks.length : 0); }, 0);
    month.mergedPunches = days.reduce(function (a, d) { return a + (d.merged || 0); }, 0);
    ...
    return {
      ...
      raw: raw,
      cut: cut,
      /* ★連打をまとめた事で 使う時刻が動いた分★（実データでは0分。0でない時は必ず見せる） */
      clean: { windowMin: ses.clean.windowMin, movedMin: ses.clean.movedMin },
