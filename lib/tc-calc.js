...
(function (root, factory) {
  ...
  function countWarnings(days, month, o, arg) {
    ...
    days.forEach(function (d) {
      ...
      /* ★働いた日に「休み」が立っている★（2026-08-15 指示役の指摘）
         ＝欠勤なのに打刻が在る／有給なのに働いている。★どちらが本当か 人にしか決められない★ので、
         勝手に片方を消さずに ★知らせる★。放っておくと ★出勤日数と欠勤日数が両方 増える★。 */
      if (d.workMin > 0 && (d.dayKind === 'absent' || d.dayKind === 'paid_leave')) {
        add('day_conflict', d.d + ' '
          + (d.dayKind === 'absent' ? '欠勤' : '有給') + 'になっていますが 打刻があります（どちらかが違います）',
        { d: d.d, dayKind: d.dayKind, workMin: d.workMin });
      }
      /* ★判定は「拘束時間」で（実労働だけで見ると1分足りない日を見逃す）★
         抜けていた分は ★重なりを1回だけ数えた offMin★ を使う（breakMin+awayMin だと二重） */
      /* ★見るのは「引いた後の休憩」★（打刻だけで見ると、既定を引いた日まで赤になる）。
         2026-08-15 に休憩を「押させる」から「既定を引く」へ変えた時に一緒に直した。 */
      var need = LAW.requiredBreakMin(d.workMin + d.offMin + d.breakMin);
      if (need > 0 && d.breakMin < need) {
        add('break_short', d.d + ' 休憩が足りません', { d: d.d, need: need, got: d.breakMin });
      }
    });

    /* ★会社の既定そのものが 法律で必要な分を下回っていないか★
       ★黙って引き上げない★（引き上げると 違法な設定が見えなくなる）。ここで知らせるだけ。
       日ごとではなく ★設定の話なので1回だけ★出す。 */
    var worst = null;
    days.forEach(function (d) {
      if (d.breakSrc !== 'default' || d.workMin <= 0) return;
      var need2 = LAW.requiredBreakMin(d.workMin + d.offMin + d.breakMin);
      if (need2 > o.breakDefaultMin && (!worst || need2 > worst.need)) {
        worst = { need: need2, d: d.d };
      }
    });
    if (worst) {
      add('break_default_short',
        '会社の休憩の既定（' + o.breakDefaultMin + '分）が、法律で必要な分（'
        + worst.need + '分）を下回っています。会社情報で直してください'
        + '（数字は勝手に変えていません）',
        { need: worst.need, got: o.breakDefaultMin, d: worst.d });
    }
    ...
    if (month.otMin + month.holidayMin >= LAW.LIMIT.singleMonthUnderMin) {
      /* ★「未満」が線★（達したら違反）。★はコードの目印なので 画面に出す文からは外す */
    ...
    /* ★法定休日を決めていない★（決めていないと 休日の割増を付けられない）
       既定では出さないが ★中では常に数える★（他の気づきと同じ扱い） */
    if (o.holidayMode === 'none') {
      add('holiday_not_set', '法定休日を決めていないので、休日の割増を付けていません（就業規則で決めてください）');
    }
    if (o.holidayMode === 'w4d4' && !o.holidayCycleStart) {
      add('holiday_cycle_missing', '4週4日制ですが 4週間の起算日が決まっていません（就業規則で決めてください）');
    }

    /* ★毎週少なくとも1日の休日（35条）★ … 1日も休みが無い週があれば数える
       ★止めない。数えて出すだけ。★ */
    var weekWork = {};
    days.forEach(function (d) {
      var wk = weekKey(d.d, o.weekStartDow);
      weekWork[wk] = weekWork[wk] || { days: 0, rest: 0 };
      weekWork[wk].days++;
      if (d.workMin <= 0) weekWork[wk].rest++;
    });
    Object.keys(weekWork).forEach(function (wk) {
      var w2 = weekWork[wk];
      /* 期間の端で week が欠けている時は数えない（7日そろっている週だけ見る） */
      if (w2.days === 7 && w2.rest < LAW.WEEKLY_HOLIDAY_DAYS) {
        add('no_weekly_holiday', wk + ' からの週に 休みが1日もありません（毎週1日が要ります）', { d: wk });
      }
    });

    /* ★4週4日★ … 起算日が在る時だけ、区切りごとに休みの日数を数える */
    if (o.holidayMode === 'w4d4' && o.holidayCycleStart) {
      var N = LAW.CYCLE_WEEKS * 7, start = dayMin(o.holidayCycleStart), blocks = {};
      days.forEach(function (d) {
        var b = Math.floor((dayMin(d.d) - start) / DAY / N);
        blocks[b] = blocks[b] || { days: 0, rest: 0, from: d.d };
        blocks[b].days++;
        if (d.workMin <= 0) blocks[b].rest++;
      });
      Object.keys(blocks).forEach(function (b) {
        var x = blocks[b];
        if (x.days === N && x.rest < LAW.CYCLE_HOLIDAY_DAYS) {
          add('cycle_short', x.from + ' からの4週間に 休みが ' + x.rest + '日しかありません（4日が要ります）', { d: x.from });
        }
      });
    }
