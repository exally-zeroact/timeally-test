...
(function (root, factory) {
  ...
  function daySentence(dayInfo, d) {
    ...
    var md = mdOf(d);
    if (x.undecided) {
      return md + ' は 決められません（' + (x.asks[0] ? x.asks[0].text : '打った物が食い違っています') + '）';
    }
  ...
  return {
    WINDOW_MIN: WINDOW_MIN, KIND_LABEL: KIND_LABEL,
    toMin: toMin, dayOf: dayOf, hmOf: hmOf, mdOf: mdOf,
    sorted: sorted, pairsOf: pairsOf,
    clean: clean, daySentence: daySentence, previewOf: previewOf,
  };
});
