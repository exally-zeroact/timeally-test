...
(function (root, factory) {
  ...
  var DENY = {
    out: '先に出勤を打ってください（打ち忘れた分は「記録へ」から足せます）',
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     ★時刻そのものを間違えた時★（2026-08-18 司さん）
       ...
       ★元の打刻は消さない・書き換えない★（直しは「元に印＋新しい時刻を1本」）
     ═══════════════════════════════════════════════════════════════════════ */
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     ★決まりは1つ★（2026-08-18 夜3 司さん
       「打刻に法が関係してるわけやないんやから シンプルイズベストでやらして」）
       ★自分の打刻は 自分で直せる・消せる。★締めた後はできない★★
     ・人が覚える言葉は ★直す／消す／足す★ の3つだけ
     ・★お願い・承認・申請は 画面に出さない★（押したら その場で入る）
     ・跡は中で残す（元の行は消さず印／誰が・いつ・何を／会社の画面に「本人が直した」）
     ★線を増やさない★＝法が絡まない所で 概念を足さない。
     ═══════════════════════════════════════════════════════════════════════ */
  var CLOSED_WHY = '締めたので直せません。会社に言ってください';
  /**
   * ★自分で直せるか★（画面も倉庫も この1つの決まりだけ見る）
   * @param {object} o o.state … 'open'|'pending'|'closed'
   * @returns {{ok:boolean, why:string}}
   */
  function canFix(o) {
    o = o || {};
    if (o.state === 'closed') return { ok: false, why: CLOSED_WHY };
    return { ok: true, why: '' };
  }
  ...
  return {
    ...
    clean: clean, stateOf: stateOf, timeOk: timeOk, canFix: canFix, CLOSED_WHY: CLOSED_WHY,
