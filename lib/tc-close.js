...
(function (root, factory) {
  ...
  /** ★画面に出す日付は M/D（曜）の1つだけ★（2026-08-21 指示役「1つに揃える」）
      ＝この lib は 画面に出す文も作るので、ここでも同じ形にする（2通りを作らない）。 */
  var DOW = ['日', '月', '火', '水', '木', '金', '土'];
  function mdDow(iso) {
    var t = String(iso || '');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(t)) return t;
    var dow = new Date(Date.UTC(+t.slice(0, 4), +t.slice(5, 7) - 1, +t.slice(8, 10))).getUTCDay();
    return t.slice(5).replace('-', '/') + '（' + DOW[dow] + '）';
  }
   ...
   *  ★人の言葉を持っていない記録は 画面に出さない★（2026-08-21 指示役が実配信で見つけた）
   *  ＝前は 知らない印（pin_set など）を ★そのまま★ 出していて、
   *    社長の画面に「2026-08-21 12:15　pin_set　田中 花子」と ★中の言葉★ が出ていた。
   *    ★空文字を返す＝出さない★（呼ぶ側は 空なら その行を描かない）。 */
  var ACTION_WORD = { close: '確定', reopen: '解除', export: '給与へ渡した' };
  function describe(row) {
    var w = ACTION_WORD[row && row.action];
    if (!w) return '';
  ...
  return {
    ...
    ACTION_WORD: ACTION_WORD, mdDow: mdDow,
