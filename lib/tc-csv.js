...
(function (root, factory) {
  ...
  function note(d) {
    var n = [];
    var cnt = {};                 /* ★同じ言葉は 1つにまとめて 回数で出す★ */
    (d.fixes || []).forEach(function (f) {
      if (f.status && f.status !== 'approved') return;
      /* ★2026-08-18 夜2★ 本人が自分で直した分は 分数を持たない（時刻の直しなので）。
         ★null分→null分 と刷らない★／★誰が直したかは 必ず残す★ */
      /* ★前と後が同じなら 分を刷らない★（2026-08-21 司さんの実機で「元は 0分 → 0分」が出た）
         ＝時刻だけを直した時は 分が動かない。★動いていない数を並べても 何も言っていない★。
         ★同じ型が 社長の画面にも在った★ので 両方 直した。 */
      if (f.beforeMin == null || f.afterMin == null || f.beforeMin === f.afterMin) {
        /* ★同じ言葉を並べない★（2026-08-22 司さんの実機で「本人が直しました」が ★4回★ 並んだ）
           ＝08-21 に「直した記録」と「まだ入っていません」で直したのと ★同じ型の3か所目★。
           ★何回 直したかは 数で言う★（「本人が直しました（4回）」）＝行が伸びない・数は消えない。 */
        var w = f.requestedBy === 'employee' ? '本人が直しました' : '会社が直しました';
        cnt[w] = (cnt[w] || 0) + 1;
        return;
      }
      n.push('直し ' + f.beforeMin + '分→' + f.afterMin + '分' + (f.reason ? '（' + f.reason + '）' : ''));
    });
    Object.keys(cnt).forEach(function (w) { n.push(cnt[w] > 1 ? w + '（' + cnt[w] + '回）' : w); });
    ...
    if (d.srcs && d.srcs.indexOf('calendar') >= 0) n.push('後から入れた');
    return n.join(' / ');
  }
